...
export default function SupplierForm() {
  const [data, setData] = useState({
    name: "", email: "", img: "", address: "", city: "", country: "", state: "", zipcode: "",
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:8000/saveSupplierApplication', data);
      setMessage(response.data.msg); 
      setSubmitted(true); 
      swal("Done!", `${response.data.msg}`, "success"); 
    } catch (error) {
      console.error(error.message + " error from posting data");
    ...
    <div className='bg-white w-full h-auto p-2 flex flex-col items-center justify-center'>
      <form onSubmit={handleSubmit} method="post" className='flex flex-col text-black w-full items-center justify-center h-4/5'>
        ...
        <div className='w-96 flex items-start flex-col justify-center p-2 m-2'>
          ...
          <input onChange={(e) => setData({ ...data, country: e.target.value })} required type="text" placeholder='Enter Country' className='p-2 w-full text-lg bg-transparent rounded bg-gray-200 outline-none' />
        </div>
        <div className='w-96 flex items-start flex-col justify-center p-2 m-2'>
        ...
        <div className='w-96 flex items-start flex-col justify-center p-2 m-2'>
          ...
          <FileBase required onDone={({ base64 }) => setData({ ...data, img: base64 })} type="file" className='text-black w-80 h-10 border-2 flex items-center justify-center border-black rounded m-4 p-2' />
        </div>
